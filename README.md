...
## create a global api class extends PPAPI

we use two calsses ：PPAPI and PPAPIListener.

create a BasicAPI abstract class extends PPAPI.
...
Then we can use this api object in our Activity, is so easy:
...
and we put the bookId input param to url as a url path, but to Activity, it not care the param should be body or url query.
...
in the parseOutut, we do nothing becasue the work has done in BasicAPI.
